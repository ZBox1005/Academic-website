...
最小化 WOR 上限限制了 OOD 情况下检测模型的统一性能，导致 WOR 的合成 OOD 数据是我们的兴趣所在。其实我们也可以用风险来衡量检测能力，即$\sup_{D \in \mathcal{D}_{OOD}}\mathcal{L}_{OE}(h;\mathcal{D})$，而我们发现基于遗憾的测量方法更好，因为它进一步**考虑了模型在面对极大的未见数据空间时的拟合能力**。
